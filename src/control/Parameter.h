...
public:
  ...
  // hmm - this "AutomationSwitch" business seems not very elegant, maybe something better can be 
  // found


  JUCE_LEAK_DETECTOR(ParameterObserver);
};
...
changed. It uses the observer pattern and can be used notify all objects that are subclasses of 
ParameterObserver. To put this to work, the interested object must register itself via 
registerParameterObserver. When this is done, it will be called back through its parameterChanged 
callback method whenever the value has changed. It may also de-register itself later via 
deRegisterParameterObserver. Such observer objects often like to maintain pointers to the 
...
public:
  ...
  means in order to avoid redundancies in the preset files.  
  mmmhhh...this seems kinda bad design.... */
...
protected:
  ...
  Array<ParameterObserver*> parameterObservers;
...
public:
  ...
  JUCE_LEAK_DETECTOR(ParameterSetObserver);
};
...
protected:
  ...
  JUCE_LEAK_DETECTOR(ParameterSetHolder);
};
