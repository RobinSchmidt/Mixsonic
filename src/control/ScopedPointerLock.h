#ifndef ScopedPointerLock_h
#define ScopedPointerLock_h

#include "../../libs/JuceIncludes.h"

/** 

This class works analogously to juce::ScopedLock, except that it works with 
a pointer-to-CriticalSection instead of a const reference. That makes it suitable to use with 
pointers to CriticalSection objects, where the pointer may also be NULL. In this case, a regular 
ScopedLock would try to dereference a NULL-pointer whereas this object here does nothing (i.e. 
it verifies that the pointer is not NULL before dereferencing it).
