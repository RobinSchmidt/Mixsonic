...
int ThreadedDrawingComponent::useTimeSlice()
{
  ...
  if( clientAreaImageIsDirty )
  {
    ...
    return 0;    // we are busy and want to be called back again as soon as possible
  }
  else
    return 300; // we are not busy, so calling back again after 300 ms is soon enough
}
...
void ThreadedDrawingComponent::drawComponent(Image *imageToDrawOnto)
{
  ...
  //g.drawFittedText(String("ThreadedDrawingComponent"), 0, 0, w, h, Justification::centred, 1);
...
bool ThreadedDrawingComponent::allocateClientAreaImage(int desiredWidth, int desiredHeight)
{
  ...
  if( clientAreaImage == NULL )
  {
    ...
    if( clientAreaImage == NULL )
    {
      showMemoryAllocationErrorBox( String("Panel::allocateClientAreaImage") );
      return false;  
    }
    else
      result = true;
