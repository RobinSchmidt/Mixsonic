...
up maximum values in the x- and y-direction, to select a currently shown range within this maximum
range and has functions for conversion between coordinates in the panel's own intrinsic coordinate 
system and component-coordinates. In order to make the UI more responsive, it's subclassed from
ThreadedDrawingComponent.

\todo maybe let this class broadcast messages, when the range has changed (in order to update
outlying scrollbars, etc.)

\todo make only a few of the set-functions virtual (i.e. intended for override), the other 
non-virtual functions are convenience functions and should call the virtual ones - 
perhaps, we should make only make setMaximumRange and setCurrentRange virtual, but in any case, 
the virtual function should be the ones which are called from PanelScroller and its subclasses)
...
public:
  ...
  /** Constructor. */
  Panel(const String &componentName = String("Panel"));   
