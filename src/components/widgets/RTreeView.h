...
\todo maybe factor out a Tree or TreeNode class for an abstraction of the data structure
...
class RTreeViewNode
{
  ...
  friend class RPopUpMenu;  
    // it is not very clean to have this as friend - maybe change this later
...
public:
  ...
  default, they are not. If you set this to true for the root node of your tree, the destruction 
  of the root node will clean up the memory for the whole tree. */
  ...
  /** Returns true when this node is a leaf node (i.e. does not have any child-nodes), 
  false otherwise. */
  ...
  /** Returns the level of the node in the tree - root nodes have level 0, their direct 
  child-nodes level 1 and so on. */
  ...
  node containing this data-pointer is found, this function will return a pointer to the respective
  node. If none is found, it returns a NULL pointer. If several nodes with the data-pointer exist, 
  it returns the first one that is found. */
  ...
  branches in order to find the next leaf node. This is useful for skipping through nodes one 
  after another. */
...
private:
  ...
  JUCE_LEAK_DETECTOR(RTreeViewNode);
};
...
public:
  ...
  argument is one of the values defined in the RTreeView::nodeClickPositions enumeration and may 
  be used to trigger different actions depending on where exactly the node was clicked. */
...
class RTreeView : public RWidget, public RScrollBarListener
{
...
public:
  ...
  RTreeView(); // needs a skin-parameter
  ...
  // \todo: get rid of these showModally functions - instead, write a class ModalComponent with a 
  // function setContentComponent that managaes all the aspects of modality
...
protected:
  ...
  /** Returns the width for a specific node, taking into account child-nodes, and their 
  children, etc. (recursively). When the ignoreOpenness parameter is false, only open nodes 
  count. */
...
class RTreeLeafNodeSelector : public RTreeView 
{
...
protected:
  ...
  JUCE_LEAK_DETECTOR(RTreeLeafNodeSelector);
};
