...
/** A class for receiving events from an RScrollBar. You can register an RScrollBarListener with an
RScrollBar using the RScrollBar::addListener() method, and it will be called when the bar's 
position changes. */

class RScrollBarListener
{
...
public:
  ...
  virtual void scrollBarMoved(RScrollBar* scrollBarThatHasMoved, double newRangeStart) = 0;
...
change the position and size of the scrollbar's 'thumb'. Registering an RScrollBarListener with 
the scrollbar will allow you to find out when the user moves it, and you can use the 
...
Timer was for?) we perhaps need an AutoRepeatingButton class

*/

class RScrollBar : public RWidget, public RButtonListener
{
...
public:
  ...
  RScrollBar(const bool isVertical, const Skin *skinToUse);
  ...
  size. Note that the maximum thumb start position is (getMaximumRangeLimit() - 
  getCurrentRangeSize()). If this method call actually changes the scrollbar's position, it will 
  trigger a call to RScrollBarListener::scrollBarMoved() for all the listeners that are registered. */
  ...
  terms of the total range, and is added or subtracted from the thumb position when the user 
  clicks an up/down (or left/right) button. */
  ...
  /** Moves the scroll bar up or down in pages. This will move the bar by a multiple of its 
  current thumb size, effectively doing a page-up or down. A positive value here will move the bar 
  down or to the right, a negative value moves it up or to the left. */
  ...
  virtual void removeListener(RScrollBarListener* const listener) throw();
    // rename them to add/removeRScrollBarListener (or maybe Observer)
