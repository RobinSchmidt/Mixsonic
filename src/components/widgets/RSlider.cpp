...
RSlider::RSlider(const Skin *skinToUse) : RWidget(skinToUse)
{
  currentValue             = 0.5;
  defaultValue             = 0.5;
  minValue                 = 0.0;
...
RSlider::~RSlider()
{
  // remove ourselves as listener from the Parameter object, such that it does not try to notify a nonexistent listener:
...
void RSlider::setValue(double newValue, const bool sendUpdateMessage, const bool sendMessageSynchronously)
{
  ...
  if(currentValue != newValue)
  {
    ...
    if( assignedParameter != NULL )
    {
      ...
      //assignedParameter->setValue(currentValue, sendUpdateMessage, sendUpdateMessage); // is the thing with the sendUpdateMessage correct?
      assignedParameter->setValue(currentValue, sendUpdateMessage);
...
void RSlider::rPopUpMenuChanged(RPopUpMenu* menuThatHasChanged)
{
  ...
  /*
  // move this into subclass AutomatableSlider:
...
void RSlider::paint(Graphics& g)
{
  ...
  g.setColour(getMixedColour(getBackgroundColor(), getMiddlegroundColor(), 0.25, 0.75));
  ...
  int x = getWidth() - getFont().getStringWidth(valueString);
  int y = handleRectangle.getY() + handleRectangle.getHeight()/2 - (int)getFont().getAscent()/2;
  g.drawText(valueString, x-4, y, getWidth(), getHeight(), Justification::topLeft, false);
  // old:
  ...
  {
    x = 0;
    y = handleRectangle.getY() - (int)getFont().getAscent() - 2;
  }
  g.drawText(sliderName, x, y, getWidth(), getHeight(), Justification::topLeft, false);
  //old:
  //x = 4;
...
void RSlider::addPopUpMidiItems()
{
  ...
  if( assignedParameter != NULL )
  {
    ...
    if( ap != NULL )
    {
      ...
      String ccString;
      if( cc > -1 )
        ccString = String(T("(currently CC#")) + String(cc) + String(T(")"));
      else
        ccString = String(T("(currently none)")); 

      int defaultCc = ap->getDefaultMidiController();
      String defaultString;
      if( defaultCc > -1 )
        defaultString = String(T("CC#")) + String(defaultCc);
      else
        defaultString = String(T("none"));
      ...
      rightClickPopUp->addItem(MIDI_LEARN,  String(T("MIDI learn ")) + ccString);
      rightClickPopUp->addItem(MIDI_MIN,    String(T("use value as lower limit (currently ")) + minString + String(T(")")));
      rightClickPopUp->addItem(MIDI_MAX,    String(T("use value as upper limit (currently ")) + maxString + String(T(")")));
      rightClickPopUp->addItem(MIDI_REVERT, String(T("revert MIDI mapping to defaults")) );
    }
  }
  */
}
...
{
  //RTextEntryField *entryField = new RTextEntryField(skin, String(getValue()));
  RTextEntryField *entryField = new RTextEntryField(getSkin(), String(getValue()));
