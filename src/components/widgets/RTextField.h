...
public:
  ...
  /** Constructor. */
  RTextField(const Skin *skinToUse, const juce::String& initialText = juce::String::empty);
  ...
  /** Tells the textfield that it should display itself without an outline or background - that is,
  it just draws its text on whatever background is already there. */  
...
public:
  ...
  the text or pastes a portion of text from the clipboard but did not yet finish typing by pressing
  enter, escape or losing focus. You may override this when you want to be informed about 
...
This class implements a textfield, the content of which may be edited by the user. Moreover, the 
class sends out notifications when the text was changed to any registered RTextEntryFieldObserver. 

\todo: BUG: selecting text with mouse doesn't work properly - probably, the bug is in 
            pixelPositionToCharacterIndex
...
public:
  ...
  /** Constructor. */
  RTextEntryField(const Skin *skinToUse, const juce::String& initialText = juce::String::empty);
...
protected:
  ...
  /** Called whenever the user has finished typing to notify our observers about the 
  text-change. */
  virtual void sendTextChangeNotificationIfChanged();

  /** Called whenever the user has typed in a new character or deletes or pastes portions of 
  text. */
  ...
  JUCE_LEAK_DETECTOR(RTextEntryField);
};
...
public:
  ...
  /** Constructor. */
  RLabeledTextEntryField(const Skin *skinToUse, const juce::String& labelText = juce::String::empty, 
...
protected:
  ...
  JUCE_LEAK_DETECTOR(RLabeledTextEntryField);
};
