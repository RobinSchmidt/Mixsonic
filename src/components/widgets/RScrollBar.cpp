...
RScrollBar::RScrollBar(const bool isVertical, const Skin *skinToUse)
: RWidget(skinToUse)
{ 
  vertical          = isVertical;
  min               = 0.0;
  ...
  if( vertical )
  {
    forwardButton  = new RClickButtonWithAutoRepeat(RButton::ARROW_DOWN, skinToUse);
    backwardButton = new RClickButtonWithAutoRepeat(RButton::ARROW_UP, skinToUse);
  ...
  {
    forwardButton  = new RClickButtonWithAutoRepeat(RButton::ARROW_RIGHT, skinToUse);
    backwardButton = new RClickButtonWithAutoRepeat(RButton::ARROW_LEFT, skinToUse);
...
void RScrollBar::applyIndents(int &x, int &y, int &w, int &h)
{
  ...
  if( vertical )
  {
    x += indent + getOutlineThickness();
    y += indent;
    w -= 2*indent + 2*getOutlineThickness();
    h -= 2*indent;
  }
  else 
  {
    x += indent;
    y += indent + getOutlineThickness();
    w -= 2*indent;
    h -= 2*indent + 2*getOutlineThickness();
  }
}
