#include "ImmediatePlaybackAudioSource.h"
...
void ImmediatePlaybackAudioSource::startPlayback(AudioFileBuffer *newBufferToPlay)
{
  // make sure that the content of the preListenBuffer (in particular, the allocated memory) is not
  // read out during we change it:
  const ScopedLock scopedLock(lock); 
  ...
  audioSampleBuffer.setSize(newBufferToPlay->getNumChannels(), newBufferToPlay->getNumSamples());
  for(int c=0; c<newBufferToPlay->getNumChannels(); c++)
    newBufferToPlay->copyTo(c, 0, audioSampleBuffer, c, 0, audioSampleBuffer.getNumSamples());
...
void ImmediatePlaybackAudioSource::restartPlayback()
{
  playPosition = 0;
}
