...
#include "../infrastructure/GlobalFunctions.h"
...
This class represents an audio-file, the data of which is buffered within an object of this class. 
This is done via inheriting from juce::AudioSampleBuffer, but in addition to the functionality of 
the base-class, this class here stores also information about the underlying file such as the 
relative path (with respect to some sample-content directory). It also provides some infastructure
to facilitate the use of this buffer for clips that share audiodata. 
...
public:
  ...
  inside a loop when you have acquired the lock already outside the loop (this is IMPORTANT - don't 
  call this function when you have not acquired the lock!). */
  ...
  /** Compares two AudioFileBuffers for equality - they regarded as equal when the filenames and 
  ...
  bool operator==(const AudioFileBuffer& otherBuffer) const  
  {
    ...
    // filenames match and file contents are sample-by-sample the same - we regard the two buffers
    // as equal:
    return true;
    */
...
protected:
  ...
  Array<AudioFileBufferUser*, CriticalSection> users;

  /** We use two separate mutex locks for accessing the audio data from the audio-thread and from 
  the graphic thread in order to not not have them block each other because there is no good reason
  why not two threads should be able to read the same data. Only when the data is changed (i.e. a 
  new audiofile is being loaded) we will aquire both locks. */
