...
AudioFileBuffer::AudioFileBuffer(const File &fileToLoadFrom) : AudioSampleBuffer(1, 0)
{
  ...
  audioFileRootDirectory   = getApplicationDirectoryAsString();
...
bool AudioFileBuffer::loadAudioDataFromFile(const File &fileToLoadFrom, bool showAlertBoxOnFail)
{
  ...
  if( reader != NULL )
  {
    ...
    {
      if( showAlertBoxOnFail )
        showMemoryAllocationErrorBox(String("AudioFileBuffer::loadAudioDataFromFile"));
      delete reader;
      return false;
    }
    ...
    reader->readSamples(intChannelPointers, reader->numChannels, 0, 0, (int) reader->lengthInSamples);
      // maybe this can be simplified by using reader->read(AudioSampleBuffer *buffer, ...
...
void AudioFileBuffer::filePathChanged(const File &newFilePath)
{
  ...
  // inform our users about the name-change:
  users.getLock().enter();
  for(int i=0; i<users.size(); i++)
    users[i]->filePathChanged(this);
  users.getLock().exit();
}
