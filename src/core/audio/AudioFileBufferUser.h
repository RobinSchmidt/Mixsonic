#ifndef AudioFileBufferUser_h
#define AudioFileBufferUser_h
...
the buffer and use it in some way (for example playing it back or displaying it on the screen). The 
AudioFileBufferUser baseclass will take care of registering with the AudioFileBuffer that is being 
used in order to allow the buffer to keep track of its referencers (for example to check in its 
destructor that it is not referenced anymore and later maybe also for garbage collection and 
...
public:
  ...
  extension. */
  virtual const String getFileNameWithoutExtension() const;
  ...
  lockUsedBufferPointer()/unlockUsedBufferPointer(). If you are also planning to access the actual 
  audio data, use acquire/realeaseAudioFileBufferReadLock(), if you are planning to change the size 
  of the buffer (which potentially involves memory-reallcoation), use 
  ...
  AudioFileBuffer::getMinMaxSamples(), or assigns both values (minValue, maxValue) to zero in case
  ...
  /** Gets called from AudioFileBuffer when its filePathChanged method was called - which is 
  supposed to happen, when the underlying file was renamed but otherwise stays the same. The 
  baseclass implementation here is empty but you may want to override it in your subclass to 
  actually do something (for example, to update some GUI element that displays the filename). */
  ...
  pointer *bufferToUse) and acquires a read-lock for the audio-data that is stored in the buffer to 
  which this pointer points (in that order). When holding these locks, you can safely do some work 
  with the buffer but do not assign a new object to the pointer and don't change the buffer's 
  size. */
  ...
  *bufferToUse points and then exits the audioFileBufferPointerLock (which locks accesses to the 
  pointer *bufferToUse). The pointer-member *bufferToUse should point to the same object as it did 
  when you called acquireAudioFileBufferReadLock() for this object. */
  virtual void releaseAudioFileBufferReadLock();

  /** Enters the lock for the member audioFileBufferPointerLock (which locks accesses to the 
  pointer *bufferToUse) and acquires a write-lock for the audio-data that is stored in the buffer to 
  which this pointer points (in that order). When holding these locks, you can safely do some work 
  with the buffer (including changing its size) but do not assign a new object to the pointer. */
  virtual void acquireAudioFileBufferWriteLock();

  /** Exits the read-lock for the audio-data that is stored in the buffer to which our member 
  *bufferToUse points and then exits the audioFileBufferPointerLock (which locks accesses to the 
  pointer *bufferToUse). The pointer-member *bufferToUse should point to the same object as it did 
  when you called acquireAudioFileBufferWriteLock() for this object. */
...
protected:
  ...
  /** A pointer to a buffer before some client code acquired the lock via the respective functions 
  (acquireAudioFileBufferReadLock(), etc.) to facilitate a check that the pointer didn't change 
  when the client code releases the lock again (releaseAudioFileBufferReadLock(), etc.) - this 
  check is for debug purposes only. */
